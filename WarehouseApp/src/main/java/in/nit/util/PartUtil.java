...
public class PartUtil {
	...
	public void generatePieChart(String location,
		...
		for(Object[] ob:list) {
			//Key -PartCode ,value-count value
		...
		chart=ChartFactory.createPieChart("PART CODE", dataset);
	...
	public void generateBarChart(String location,
		...
		//b. Convert Dataset data into JFreeChart object using ChartFactory class
		chart=ChartFactory.createBarChart("PART CODE","PART CODES", "COUNT", dataset);
		
		//C.Convert JFreeChart object into Image using ChartUtil class
		
		try {
			ChartUtils.saveChartAsJPEG(new File(location+"/partB.jpg"),chart, 500,350);
		} catch (IOException e) {
