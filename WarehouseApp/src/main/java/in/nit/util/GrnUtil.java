...
public class GrnUtil {
	...
	public void generatePieChart(String location,
		...
		chart=ChartFactory.createPieChart("GRN CODE", dataset);
		
		//C.Convert JFreeChart object into Image using ChartUtil class
		try {
			ChartUtils.saveChartAsJPEG(new File(location+"/grnA.jpg"), chart,400 , 300);
		} catch (IOException e) {
	...
	public void generateBarChart(String location,
		...
		//b. Convert Dataset data into JFreeChart object using ChartFactory class
		chart=ChartFactory.createBarChart("GRN CODE","GRN TYPES", "COUNT", dataset);
		
		//C.Convert JFreeChart object into Image using ChartUtil class
		
		try {
			ChartUtils.saveChartAsJPEG(new File(location+"/grnB.jpg"),chart, 500,350);
		} catch (IOException e) {
